...
def isBalanced(s):
    ...
    for i in range(0, len(s)):  # For loop to iterate over the string given in the parameter.
        if s[i:i + 1] == '(' or s[i:i + 1] == '{' or s[i:i + 1] == '[': # Check for a starting bracket
            bracket_stack.append(s[i:i + 1]) # If starting bracket is found, push it into the stack
        else: # If we encounter a closing bracket
            if len(bracket_stack) == 0: # No starting bracket to match closing bracket, so incorrect.
                return 'NO'
            start_bracket = bracket_stack.pop() # Pop the starting bracket
            if start_bracket == '(': # Check if parenthesis are balanced
                if s[i:i + 1] != ')':
                    return 'NO'
            if start_bracket == '{': # Check if parenthesis are balanced
                if s[i:i + 1] != '}':
            ...
            if start_bracket == '[': # Check if parenthesis are balanced
                if s[i:i + 1] != ']':
                    return 'NO'
    if len(bracket_stack) != 0: # Starting bracket without closing bracket
        return 'NO'
    return 'YES'
